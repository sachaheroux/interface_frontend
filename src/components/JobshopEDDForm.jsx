...
const JobshopEDDForm = () => {
  ...
  const calculateOptimization = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/jobshop/edd`, {
        method: 'POST',
        headers: {
      ...
      try {
        const ganttResponse = await fetch(`${API_URL}/jobshop/edd/gantt`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const downloadGanttChart = () => {
    if (ganttUrl) {
      ...
      link.href = ganttUrl;
      link.download = 'diagramme_gantt_jobshop_edd.png';
  ...
  return (
    <div className={styles.algorithmContainer}>
      ...
      <div className={styles.header}>
        <h1 className={styles.title}>Jobshop - Algorithme EDD</h1>
