...
export default function FMSSacADosForm() {
  ...
  const calculerSacADos = async () => {
    ...
    try {
      ...
      const response = await fetch(`${config.API_URL}/fms/sac_a_dos`, {
        method: "POST",
      ...
      try {
        const chartResponse = await fetch(`${config.API_URL}/fms/sac_a_dos/chart`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(donnees)
        });
  ...
  return (
    <div className={styles.algorithmContainer}>
      <div className={styles.header}>
        <h1 className={styles.title}>FMS - Sac à Dos avec Programmation Dynamique</h1>
        <p className={styles.subtitle}>
          Optimisation de production par mémorisation et récurrence (solution optimale)
        </p>
      </div>
      ...
      >
        {loading ? "Calcul en cours..." : "Optimiser avec Programmation Dynamique"}
      </button>
          ...
          {graphique && (
            <div className={styles.solutionDetails}>
              ...
              <div className={styles.chartContainer}>
                <img 
                  src={graphique} 
                  alt="Graphique d'analyse FMS Sac à Dos Programmation Dynamique" 
