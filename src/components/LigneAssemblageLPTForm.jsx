...
import styles from './LigneAssemblageLPTForm.module.css';
...
const LigneAssemblageLPTForm = () => {
  ...
  const calculateOptimization = async () => {
    ...
    try {
      ...
      try {
        const chartResponse = await fetch(`${API_URL}/ligne_assemblage/lpt/chart`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const downloadChart = () => {
    if (chartUrl) {
      ...
      link.href = chartUrl;
      link.download = 'equilibrage_lpt.png';
  ...
  const handleImportExcel = async (formData, fileName) => {
    ...
    try {
      const response = await fetch(`${API_URL}/ligne_assemblage/lpt/import-excel`, {
        method: 'POST',
        body: formData
      });
  ...
  return (
    <>
      ...
      <div className={styles.header}>
        <h1 className={styles.title}>Ligne d'assemblage - LPT</h1>
        <p className={styles.subtitle}>
          Équilibrage par règle du temps de traitement le plus long (Longest Processing Time)
        </p>
      </div>
      ...
      <div className={styles.section}>
        ...
        <ExcelExportSectionLigneAssemblage
          ...
          algorithmName="LPT"
          API_URL={API_URL}
          algorithmEndpoint="ligne_assemblage/lpt"
        />
      </div>
      ...
      <button
        ...
        type="button"
      >
        {isCalculating ? 'Calcul en cours...' : 'Calculer l\'équilibrage LPT'}
      </button>

      {/* Résultats */}
      {result && (
        <div className={`${styles.section} ${styles.resultsSection}`}>
          <h2 className={styles.resultsTitle}>Résultats de l'équilibrage LPT</h2>
          ...
          <div className={styles.stationsDetails}>
            <h4>Configuration des stations par LPT</h4>
      ...
      {chartUrl && (
        <div className={`${styles.section} ${styles.chartSection}`}>
          <div className={styles.chartHeader}>
            <h3>Graphiques d'analyse LPT</h3>
          </div>
...
export default LigneAssemblageLPTForm; 